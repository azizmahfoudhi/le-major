...
export default function RessourcesClient({ 
  ...
  const matieres = useMemo(() => {
    const unique = Array.from(new Set(initialResources.map(c => c.matiere))).sort();
    return unique;
  }, [initialResources]);

  // Filter content by selected matière
  const filtered = useMemo(() => {
    if (!filterMatiere) return initialResources;
    return initialResources.filter(c => c.matiere === filterMatiere);
  }, [initialResources, filterMatiere]);
    ...
    <div className="space-y-6">
      ...
      {matieres.length > 1 && (
        <div className="flex items-center gap-3">
          ...
          >
            <option value="">Toutes les matières ({initialResources.length})</option>
            {matieres.map(m => (
              <option key={m} value={m}>
                {m} ({initialResources.filter(c => c.matiere === m).length})
              </option>
            ))}
          </select>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-card border border-gray-100 overflow-hidden">
        {filtered.length > 0 ? (
          <DataTable 
            ...
            actions={(item) => (
              <div className="flex justify-end gap-2">
                <Link href={`/admin/ressources/${item.id}/modifier`}>
          ...
          <div className="p-12 text-center text-gray-500 flex flex-col items-center">
            ...
            <p className="mb-4">
              {filterMatiere ? `Aucune ressource pour la matière "${filterMatiere}".` : "Aucune ressource n'a été créée."}
            </p>
            {!filterMatiere && (
              <Link href="/admin/ressources/nouveau">
                <Button><Plus className="w-4 h-4 mr-2" />Créer la première ressource</Button>
              </Link>
            )}
          </div>
        )}
