...
import { Save, ArrowLeft, Loader2, FolderOpen } from 'lucide-react';
...
export default function ResourceForm({ chapters, initialData }: ResourceFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 max-w-4xl">
      ...
      <div className="flex items-center gap-3 p-3 bg-blue-50 border border-blue-100 rounded-lg text-sm text-blue-700">
        ...
        <span>Ce contenu sera créé en tant que <strong>Ressource Annexe</strong> et publié immédiatement.</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2 md:col-span-2">
          ...
          <Input 
            id="title" 
            name="title" 
            placeholder="Ex: Formulaire de Mathématiques, Lien vers une vidéo YouTube..." 
            required 
      ...
      <div className="space-y-2">
        <label htmlFor="mdx_content" className="text-sm font-medium text-navy-900 flex justify-between">
          <span>Contenu de la ressource (Format MDX)</span>
          <span className="text-gray-400 font-normal">Prend en charge KaTeX ($$...$$) et l'intégration de liens</span>
        </label>
        <textarea 
          id="mdx_content"
          name="mdx_content"
          ...
          placeholder={"# Titre de la ressource\n\nVous pouvez intégrer des liens, du texte, des images ou même des vidéos ici.\n\n[Cliquez ici pour télécharger le PDF](https://lien.com/fichier.pdf)"}
        />
      </div>
        ...
        <Button type="submit" disabled={isSubmitting}>
          {isSubmitting ? (
            ...
            <>
              ...
              {initialData ? 'Enregistrer les modifications' : 'Publier la ressource'}
            </>
          )}
        </Button>
      </div>
